...
// TODO fix all TODOs below
// Avoid too many copies of data.
// Figure out why I only have to check for SELF_TYPE in some places.
// Make sure methods start with lowercase. Classes may have start with uppercase but I am not sure.

bool method_class::can_override(const method_class& other) const {
    if (name != other.name)
    {
        return false;
    }
    ...
    if (formals->len() != other.formals->len())
    {
        return false;
    }
    ...
    {
        if (formals->nth(i)->get_type_decl() != other.formals->nth(i)->get_type_decl())
        {
            return false;
        }
        i = formals->next(i);
...
Symbol attr_class::get_type() const {
    return type_decl;
}

Symbol method_class::get_type() const {
    return return_type;
}
...
void method_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class)
{
    // TODO check that return type is a subclass of the class type
    ...
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Method " << name << " returns type " << actual_return_type << " which does not conform to the return type " << return_type << "." << endl;
    }
}

Symbol assign_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class)
{
    auto rtype = expr->typecheck(object_env, classtable, current_class);

    set_type(rtype);

    // in here I ave to check if rtype is in the class table instead of object_env
    ...
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Symbol " << name << " not found in class table." << endl;
        ltype = rtype;
        // TODO: Perhaps type becomes Object?
    }
    else if (!classtable.conforms(rtype, *ltype_ptr, current_class))
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Method " << name << " returns type " << rtype << " which is not a subclass of " << *ltype_ptr << "." << endl;
        ltype = rtype;
    }
    else
    {
...
Symbol static_dispatch_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class)
{
    auto expr_type = expr->typecheck(object_env, classtable, current_class);

    auto result_type = Symbol(Object);

    //need to check if type_name is SELF_TYPE, and if so, use current_class
    ...
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Symbol " << type_name << " not found in class table." << endl;
    }
    else if (!classtable.conforms(expr_type, type_name, current_class))
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Expression type " << expr_type << " is not a subclass of " << type_name << "." << endl;
    }
    else if (!classtable.has_method(type_name, name))
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Method " << name << " not found in class table." << endl;
    }
    else
    {
        ...
        {
            classtable.semant_error(classtable.get_class(current_class)) << "Method " << name << " has " << actual->len() << " arguments, but " << formals->len() << " are expected." << endl;
        }
        ...
        while (actual->more(i))
        {
            ...
            if (has_correct_argument_len)
            {
                ...
                {
                    classtable.semant_error(classtable.get_class(current_class)) << "Argument " << i << " has type " << actual_type << " which is not a subclass of " << formals->nth(i)->get_type_decl() << "." << endl;
                }
            }

            i = actual->next(i);
        }
        // need to check that returns types are compatible. What to do about SELF_TYPE?
...
Symbol dispatch_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class)
{
    auto expr_type = expr->typecheck(object_env, classtable, current_class);
    ...
    if (!classtable.has_method(sanitized_expr_type, name)) // This is looking in the wrong place.
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Method " << name << " not found in class table." << endl;
    }
    else
    {
        ...
        while (actual->more(i))
        {
            ...
            if (has_correct_argument_len)
            {
                if (!classtable.conforms(current_actual_type, formals->nth(i)->get_type_decl(), current_class))
                {
                    classtable.semant_error(classtable.get_class(current_class)) << " Attribute " << i << " in method " << name << " does not conform to the formal parameter type " << formals->nth(i)->get_type_decl() << "." << endl;
                }
            }
...
Symbol cond_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class)
{
    ...
    if (pred_type != Bool)
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Cond operation requires a Bool predicate. but got " << pred_type << "." << endl;
    }
    else
    {
...
Symbol loop_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class)
{
    auto pred_type = pred->typecheck(object_env, classtable, current_class);
    ...
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Loop operation requires a Bool predicate." << endl;
    }
...
Symbol typcase_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class)
{
    ...
    while (cases->more(i))
    {
        ...
        {
            classtable.semant_error(classtable.get_class(current_class)) << "Case " << cases->nth(i)->get_name() << " has the same type as another case." << endl;
            set_type(Object);
            return Object;
        }
    ...
    if (!classtable.conforms(lub, expr_type == SELF_TYPE ? current_class : expr_type, current_class)) // TODO Why do I only have to check for SELF_TYPE here? something is wrong.
    {
        classtable.semant_error(classtable.get_class(current_class)) << "AAExpression type " << expr_type << " is not a subclass of " << lub << "." << endl;
    }
    else
    {
        result_type = lub;
...
Symbol branch_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class)
{
    auto expr_type = expr->typecheck(object_env, classtable, current_class);
    ...
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Type " << type_decl << " not found in class table." << endl;
    }
    else if (!classtable.conforms(expr_type, type_decl, current_class))
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Expression type " << expr_type << " is not a subclass of " << type_decl << "." << endl;
    }
    else
    {
...
Symbol block_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class)
{
    auto i = body->first();
...
Symbol let_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    ...
    //what happens with the identifier? can I

    // I have to deal with self_type and what happens if one or both are self_type. do this later.
    if (type_decl != SELF_TYPE && !classtable.is_valid_class(type_decl))
    {
        classtable.semant_error(classtable.get_class(current_class)) << "In let expression, type " << type_decl << " not found in class table." << endl;
    }
    else if (type_decl != SELF_TYPE && init_type != Symbol(No_type) && !classtable.conforms(init_type, type_decl, current_class))
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Initialization type " << init_type << " is not a subclass of " << type_decl << "." << endl;
    }
...
Symbol plus_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    ...
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Plus operation requires two Int arguments." << endl;
        type = Symbol(Object);
    }
...
Symbol sub_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    ...
    if (e1_type != Symbol(Int) || e2_type != Symbol(Int))
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Sub operation requires two Int arguments." << endl;
        type = Symbol(Object);
    }
...
Symbol mul_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    ...
    if (e1_type != Symbol(Int) || e2_type != Symbol(Int))
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Mul operation requires two Int arguments." << endl;
        type = Symbol(Object);
    }
...
Symbol divide_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    auto e1_type = e1->typecheck(object_env, classtable, current_class);
    auto e2_type = e2->typecheck(object_env, classtable, current_class);
    auto type = Symbol(Int);
    ...
    if (e1_type != Symbol(Int) || e2_type != Symbol(Int))
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Divide operation requires two Int arguments." << endl;
        type = Symbol(Object);
    }
...
Symbol neg_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    ...
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Neg operation requires an Int argument." << endl;
        type = Symbol(Object);
    }
...
Symbol leq_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class)
{
    ...
    if (e1_type != Symbol(Int) || e2_type != Symbol(Int))
    {
        classtable.semant_error(classtable.get_class(current_class)) << "Leq operation requires two Int arguments." << endl;
        type = Symbol(Object);
    }
...
Symbol comp_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    auto rtype = e1->typecheck(object_env, classtable, current_class);

    // TODO nikosa what if rtype is not Bool?
...
Symbol int_const_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    auto type = Symbol(Int);
    set_type(type);
    return type;
}

Symbol bool_const_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    auto type = Symbol(Bool);
    set_type(type);
    return type;
}

Symbol string_const_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    auto type = Symbol(Str);
    set_type(type);
    return type;
}

Symbol new__class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
...
Symbol isvoid_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    auto rtype = e1->typecheck(object_env, classtable, current_class);
...
Symbol no_expr_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    set_type(Symbol(No_type));
    return Symbol(No_type);
}

Symbol object_class::typecheck(SymbolTable<Symbol, Symbol>& object_env, const ClassTable& classtable, Symbol current_class) {
    ...
    for (auto c : no_children_classes)
    {
        ...
        if (class_children_it != class_to_children_map.end())
        {
            ...
            if (class_children.size() > 0)
            {
                ...
                {
                    semant_error(class_name_to_class_map[child]) << child
                    << " cannot inherit from " << c << "." << endl;
                }
            }
        }
    }
    ...
    for (auto c : class_to_parent_map) {
        ...
        while (parent_class_symbol != No_class) {
            ...
            if (visiting.find(parent_class_symbol) != visiting.end()) {
                semant_error(class_name_to_class_map[current_class_symbol]) << "Cycle detected at: " << parent_class_symbol << "." << endl;
                break;
            }
...
// TODO remove explicit std::pair initialization, use list initializers.
...
void ClassTable::create_class_to_methods_map() {
    ...
    for (auto c : class_name_to_class_map) {
        ...
        while (features->more(i)) {
            ...
            if (method_ptr != nullptr) {
                ...
                if (class_to_method_names_map[class_symbol].find(method_ptr->get_name()) != class_to_method_names_map[class_symbol].end()) {
                    semant_error(class_name_to_class_map[class_symbol]) << "Method " << method_ptr->get_name() << " is already defined." << endl;
                }

                class_to_method_names_map[class_symbol].insert(method_ptr->get_name());
                class_method_pair_to_method_obj_map.insert(std::make_pair(std::make_pair(class_symbol, method_ptr->get_name()), *method_ptr));
            }
            i = features->next(i);
        }
    }
    ...
    for (auto c : class_to_method_names_map) {
        ...
        for (auto method_name : class_method_names) {
            ...
            if (return_type != SELF_TYPE && class_name_to_class_map.find(return_type) == class_name_to_class_map.end()) {
                semant_error(class_name_to_class_map[class_symbol]) << "Return type " << return_type << " does not exist in class table." << endl;
            }
            ...
            while (formals->more(j)) {
                ...
                if (class_name_to_class_map.find(formal_name) != class_name_to_class_map.end()) {
                    semant_error(class_name_to_class_map[class_symbol]) << "Formal name " << formal_name << " is already defined in class table." << endl;
                }

                // Formal name must not be self.
                if (formal_name == self) {
                    semant_error(class_name_to_class_map[class_symbol]) << "Self is not allowed as an argument name." << endl;
                }
                // Formal name must not be duplicate.
                else if (formal_names.find(formal_name) != formal_names.end()) {
                    semant_error(class_name_to_class_map[class_symbol]) << "Duplicate formal name: " << formal_name << endl;
                }
                ...
                if (class_name_to_class_map.find(formal_type) == class_name_to_class_map.end()) {
                    semant_error(class_name_to_class_map[class_symbol]) << "Formal type " << formal_type << " does not exist in class table." << endl;
                }
                j = formals->next(j);
            }
        }
    ...
    for (auto c : class_to_method_names_map) {
        ...
        while (parent_class_symbol != No_class && parent_class_symbol != Symbol()) {
            ...
            std::set_intersection(class_method_names.begin(), class_method_names.end(), parent_class_method_names.begin(), parent_class_method_names.end(), std::back_inserter(common_method_names));
            for (auto method_name : common_method_names)
            {
                auto method_obj = class_method_pair_to_method_obj_map.at(std::make_pair(class_symbol, method_name));
                if (!method_obj.can_override(class_method_pair_to_method_obj_map.at(std::make_pair(parent_class_symbol, method_name))))
                {
                    semant_error(class_name_to_class_map[class_symbol]) << "Method " << method_name << " overrides with a different signature." << endl;
                }
            }
...
void ClassTable::verify_attributes() {
    ...
    for (auto c : class_name_to_class_map) {
        ...
        while (features->more(i)) {
            ...
            if (attr_ptr != nullptr) {
                ...
                if (attr_name == self) {
                    semant_error(class_name_to_class_map[class_symbol]) << "Self is not allowed as an attribute name." << endl;
                }
                // Need to verify no 2 attributes have the same name.
                if (class_to_attr_names_map[class_symbol].find(attr_name) != class_to_attr_names_map[class_symbol].end()) {
                    semant_error(class_name_to_class_map[class_symbol]) << "Attribute " << attr_name << " is already defined." << endl;
                }
                ...
                if (attr_type != prim_slot && class_name_to_class_map.find(attr_type) == class_name_to_class_map.end()) {
                    semant_error(class_name_to_class_map[class_symbol]) << "Attribute type " << attr_type << " does not exist in class table." << endl;
                }
            }
            i = features->next(i);
        }
    ...
    // At this point, each class has a correct set of attributes, if no error has been raised above.
    // Ideally we would be constructing a map from class name to all attributes in the class, including inherited attributes.
    // We would then keep that for later type checking.
    // However, for now, we will later be using other data structures to conform to the guidelines for this project.
        ...
        while (parent_class_symbol != No_class && parent_class_symbol != Symbol()) {
            ...
            if (collided_attributes.size() > 0) {
                for (auto attr_name : collided_attributes) {
                    semant_error(class_name_to_class_map[class_symbol]) << "Attribute " << attr_name << " in class " << class_symbol << " is already defined in parrent class " << parent_class_symbol << endl;
                }
            }
            parent_class_symbol = class_to_parent_map[parent_class_symbol];
        }
...
bool ClassTable::has_method(Symbol class_name, Symbol method_name) const {
    ...
    while (current_class != No_class && current_class != Symbol()) {
        if (class_to_method_names_map.at(current_class).find(method_name) != class_to_method_names_map.at(current_class).end()) {
            return true;
        }
...
method_class ClassTable::get_method(Symbol class_name, Symbol method_name) const {
    ...
    // TODO better explanation for this exception.
    // It's not supposed to happen if the implementation of semant is correct.
