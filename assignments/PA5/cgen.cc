...
void CgenClassTable::code_prototype_objects()
{
  apply_in_tag_order([](CgenNodeP node, int tag, ostream& str) {
    node->code_prototype_object(tag, str);
  });
}
...
void CgenClassTable::code_class_name_table()
{
  ...
  apply_in_tag_order([](CgenNodeP node, int tag, ostream& str) {
    node->code_class_name_entry(tag, str);
  });
  str << std::endl;
}
...
{
  apply_in_tag_order([](CgenNodeP node, int tag, ostream& str) {
    node->code_dispatch_entry(tag, str);
  });
}
...
void CgenClassTable::code_class_obj_table()
{
  ...
  apply_in_tag_order([](CgenNodeP node, int tag, ostream& str) {
    node->code_class_obj_entry(tag, str);
  });
  str << std::endl;
}
...
CgenClassTable::CgenClassTable(Classes classes, ostream& s) : str(s), predefined_method_classes {Object, IO, Str}
{
  stringclasstag = 4;
  intclasstag =    2;
  boolclasstag =   3;
...
//
  add_special_class(No_class,
	new CgenNode(class_(No_class,No_class,nil_Features(),filename),
			    Basic,this));
  add_special_class(SELF_TYPE,
	new CgenNode(class_(SELF_TYPE,No_class,nil_Features(),filename),
			    Basic,this));
  add_special_class(prim_slot,
...
void CgenClassTable::construct_attribute_tables()
{
  ...
  apply_from_root([](CgenNodeP node) {
    node->construct_attribute_table();
  });
}
...
void CgenNode::construct_attribute_table()
{
  ...
  while (features->more(i))
  {
    ...
    attribute_offset_map[attr_ptr->name] = attribute_table.size() - 1;
    i = features->next(i);
  }
...
void CgenNode::construct_dispatch_table()
{
  ...
  while (features->more(i))
  {
    ...
    auto it = dispatch_offset_map.find(method_ptr->name);
    ...
    else
    {
      ...
      dispatch_offset_map[method_ptr->name] = dispatch_table.size() - 1;
    }
    i = features->next(i);
  }
...
void CgenNode::code_prototype_object(int tag, ostream& str)
{
  ...
  str << WORD; emit_disptable_ref(name, str); str << std::endl;

  for (auto [symbol, attr_ptr] : attribute_table)
  {
    str << WORD;
    // Need to first find the class of the attribute.
    if (attr_ptr->type_decl == Int)
    {
      inttable.lookup_string("0")->code_ref(str);
    }
    else if (attr_ptr->type_decl == Bool)
    {
      falsebool.code_ref(str);
    }
    else if (attr_ptr->type_decl == Str)
    {
    ...
    str << " # attribute " << attr_ptr->name->get_string() << std::endl;
  }
...
void CgenNode::code_dispatch_entry(int tag, ostream& str)
{
  ...
  for (auto [symbol, method_ptr] : dispatch_table)
  {
    ...
    str << WORD; emit_method_ref(symbol, method_ptr->name, str); str << std::endl;
  }
  str << std::endl;
}
...
void CgenNode::code_object_initializer(int tag, ostream& str, CgenClassTableP class_table, Symbol class_name)
{
  ...
  // This is needed because trap.handler calls *_init directly without setting $s0.
  ...
  for (size_t offset = 0; offset < attribute_table.size(); offset++)
  {
    ...
    Tracker locals_env {};  // Standard tracker since we only push RA (like normal methods)
    str << "# a " << symbol << endl;
    if (dynamic_cast<no_expr_class*>(attr_ptr->init) == nullptr) {
      str << "# c " << symbol << endl;
      attr_ptr->init->code(str, class_table, class_name, locals_env);
...
void CgenNode::code_class_method(ostream& s, CgenClassTableP class_table, Symbol class_name)
{
  for (auto i = features->first(); features->more(i); i = features->next(i)) {
    ...
    for (int j = method_ptr->formals->first(); method_ptr->formals->more(j); j = method_ptr->formals->next(j)) {
      formal_stack.push(method_ptr->formals->nth(j)->get_name());
    }
    ...
    s << std::endl;
    emit_method_def(name, method_ptr->name, s);
    emit_move(FP, SP, s);
    emit_push(RA, s);
    method_ptr->expr->code(s, class_table, class_name, locals_env);

    locals_env.exitscope();

    emit_pop(RA, s);
    s << "# remove formals " << method_ptr->formals->len() << method_ptr->name->get_string() << std::endl;
    emit_addiu(SP, SP, method_ptr->formals->len() * 4, s);
    emit_return(s);
  }
  s << std::endl;
}
...
void assign_class::code(ostream &s, CgenClassTableP class_table, Symbol class_name, Tracker& locals_env) {
  ...
  if (it != parameter_offsets.end()) {
    ...
    // need to record the assignment for garbage collection.
...
// TODO reduce duplicity in below methods.
void static_dispatch_class::code(ostream &s, CgenClassTableP class_table, Symbol class_name, Tracker& locals_env) {
  ...
  this->expr->code(s, class_table, class_name, locals_env); // ACC has the object, with the dispatch table.
  emit_bne(ACC, ZERO, continue_label, s);

  // TODO add correct linenumber
  ...
  // For static dispatch, use the specified type's dispatch table directly
...
void dispatch_class::code(ostream &s, CgenClassTableP class_table, Symbol class_name, Tracker& locals_env) {
  ...
  // Evaluate and push arguments in order (first arg pushed first, ends up deepest)
  for (int i = actual->first(); actual->more(i); i = actual->next(i)) {
    actual->nth(i)->code(s, class_table, class_name, locals_env);
    emit_push(ACC, s);
  }
  ...
  this->expr->code(s, class_table, class_name, locals_env); // ACC has the object, with the dispatch table.
  emit_bne(ACC, ZERO, continue_label, s);
  // Load file name and line number here.
  emit_load_address(ACC, "str_const0", s);
  emit_load_imm(T1, 0, s);
  emit_jal("_dispatch_abort", s);
  emit_label_def(continue_label, s);
  ...
  emit_move(SELF, ACC, s);
  if (type == SELF_TYPE) {
    // We don't need to know the actual type in this case.
    // This is because we have made sure to keep the same offset for methods,
    // even in derived classes (regardless of whether they are overridden or not).
    type = class_name;
  }
  emit_load(T1, 2, ACC, s); s << " # Load dispatch table address " << this->name->get_string() << endl;
...
void typcase_class::code(ostream &s, CgenClassTableP class_table, Symbol class_name, Tracker& locals_env) {
  ...
  // I have to start with the most specific class first.
  // I can do that by checking the depth.
  // There are many ways to organize this, epecially if some classes are unrelated.
  ...
  for (int i = cases->first(); cases->more(i); i = cases->next(i)) {
    ...
    int current_depth {};
    auto current_branch = current_case->type_decl;
  ...
  emit_load(T1, 0, ACC, s); // load type
  ...
  for (const auto& [branch, _] : branch_depths) {
    s << "\t# start branch: " << branch->name->get_string() << std::endl;
    const int branch_exit_label = CgenClassTable::get_next_label();
    const int branch_do_label = CgenClassTable::get_next_label();

    std::vector<CgenNodeP> descendants {class_table->get_node_from_symbol(branch->type_decl)};
    ...
    locals_env.enterscope();
    locals_env.add_local_id(branch->name);

    visited_classes.insert(branch->type_decl);

    branch->expr->code(s, class_table, class_name, locals_env);

    emit_addiu(SP, SP, 4, s);  // Discard branch result.
...
void let_class::code(ostream &s, CgenClassTableP class_table, Symbol class_name, Tracker& locals_env) {
  ...
  // needs to create a new scope for the new temporary.
  locals_env.enterscope();
...
void plus_class::code(ostream &s, CgenClassTableP class_table, Symbol class_name, Tracker& locals_env) {
  ...
  emit_pop(ACC, s);
  emit_jal_raw(s); emit_method_ref(Object, ::copy, s); s << endl;
  emit_load(T1, DEFAULT_OBJFIELDS, ACC, s);
...
void new__class::code(ostream &s, CgenClassTableP class_table, Symbol class_name, Tracker& locals_env) {
  ...
  {
    // The type is the same as the type of SELF.
    // Get the prototype object and init method from CLASSOBJTAB.
  ...
  else
  {
    ...
    emit_jal_raw(s); emit_method_ref(Object, ::copy, s); s << endl;
    emit_move(SELF, ACC, s);
    emit_jal_raw(s); emit_init_ref(type_name, s); s << endl;
  }
  emit_pop(SELF, s);
  emit_pop(FP, s);
}
...
void object_class::code(ostream &s, CgenClassTableP class_table, Symbol class_name, Tracker& locals_env) {
  ...
  // TODO move the custom defined scope map.
